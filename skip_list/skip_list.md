...
* 增：为了保证插入数据后数组仍保持有序性，可使用二分查找确定插入位置，查找过程的时间复杂度为 O(logN)，插入过程中，比插入数据小的记录都需要移动，时间复杂度为 O(N)，所以总体时间复杂度为 O(N)
* 删：使用二分查找确定待删除的数据，如有需要再移动部分数据，所以总体负责度为 O(N)，分析：查找过程是 O(logN)
* 改：使用二分查找确定待修改的数据，无需移动记录，所以时间复杂度为O(logN)
* 查：二分查找，时间复杂度为 O(logN)
...
## 2 跳跃表
跳跃表是一种基于有序链表的扩展，而链表不能使用二分查找法，所以跳跃表也无法使用二分法来提高查找效率。

类似 MySQL 的索引，可以将链表中的部分节点作为索引，并将这些节点构成新的链表，从而减少遍历比较的节点数。
...
与二叉搜索树相比，跳跃表的优点：维持平衡的成本较低，跳跃表通过随机方法，而二叉搜索树需要调整二叉树的结构，需要移动大量记录。
